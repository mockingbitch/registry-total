...
<h2>Thêm mới xe</h2>
<div class="card mb-4">
    <form action="{{route('xe.import')}}" method="post" enctype="multipart/form-data">
        @csrf
        <input type="file" name="file">
        <button type="submit" class="btn btn-primary">Nhập Excel</button>
    </form>
    <form class="mx-4 pt-4" action="{{route('xe.create.post')}}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="form-group mt-4">
            <label for="inputName">Chủ xe @if ($errors->has('tenChuXe'))<p class="text-error">*{{$errors->first('tenChuXe')}}</p>@endif</label>
            <input type="text" name="tenChuXe" class="form-control" id="inputName" aria-describedby="nameHelp" value="{{Request::old('tenChuXe')}}">
        </div>
        <div class="form-group mt-4">
